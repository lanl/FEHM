...
c gaz 040621 added qtotin below      
     ...
     &  tmin_co2wh_tabl,tmax_co2wh_tabl,
     &  pmin_co2wh_tabl,pmax_co2wh_tabl 
      use compart
      use comriv
...
7009      format 
     &  ('>>> critical ',a11,': true',1p, g16.10,' EOS table ',g16.10) 
...
c gaz 112721 write info regarding EOS data crit pts and true crit pts
 ...
 6200 format
     & (/,'>>> co2 property interpolation table -',3x, a100) 
 6300 format
...
c**** solve for heat and mass transfer solution ****
 ...
 6030           format
     &          (/, 1x, '**** allotted time gone, terminating ',
     &           'run : restart = ', a, ' ****')
...
c gaz 051424      
         if(i_vtk.ne.0.and.ivf.ne.-1) then
          call contr(2)
         else if(i_vtk.ne.0.and.ifdm_elem.eq.1) then
          call contr(2)
         endif
