...
c new cappilary pressure routine
	...
	drl_h2o4 = 0.0
      ...
      do mid=1,neq
         mi=mid+ndummy
         call rlperm_hyd(ndummy,0,mi,rl_h2o(mi),0.0d0,0.0d0,
     &           drl_h2o3(mi),drl_h2o4(mi),rl_meth(mi),
     &           0.0d0,0.0d0,drl_meth3(mi),drl_meth4(mi))

      enddo 
c get hydrate dissociation rate
       allocate(skhyd_temp(n0))
       allocate(dskhydpf(n0))
       allocate(dskhydtf(n0))
       allocate(dskhydmf(n0))
       allocate(dskhydwf(n0)) 
       ...
       dskhydmf = 0.0d0
       dskhydwf = 0.0d0 
c this call will form hydrate kinetic rate terms       
       call hyddiss(0,1)
...
c    &                              rlwm,drlwmp,drlwmt,drlwm3,dum6)
            rlwm = rl_h2o(mi)
             ...
             drlwmp = 0.0
             drlwmt = 0.0
c now calculated in call to hyddiss
...
c            call hydrate_properties(3,1,pl,tl,fracw(mi),frachyd(mi),
c     &                    skhyd(mi),dskhydp,dskhydt,dskhydw,dskhydm)
c
c     save rate and derivatives for enthalpy calculation (passed through commeth)
c

      source_hyd_temp = skhyd_temp(mi)
      dskhydp = dskhydpf(mi)
      dskhydt = dskhydtf(mi)
      dskhydw = dskhydwf(mi)
      dskhydm = dskhydmf(mi)
...
c     form dissociation flow term
            ...
            skwhyd(mi) = -ratw*source_hyd_temp
            qhhyd(mi)  = 0.0d0
...
c note rlwm used for both phases of methane 
     ...
     &              -rol*xrl/xvisl**2*dvislt)
              dilw(mi)=prf*rlwm*rol*drlw/xvisl+prf*drlwm3*rol*xrl/xvisl
...
c     modify flow terms for new upwind scheme
     ...
     &             -rov*xrv/xvisv**2*dvisvt)
              divw(mi)=prf*rlwm*rov*drvw/xvisv+prf*drlwm3*rov*xrv/xvisv
...
c     form dissociation flow term
            ...
            skmhyd(mi) = -ratgas*source_hyd_temp
            qhhyd(mi)  = 0.0d0
...
c note rgwm used for both phases of methane 
     ...
     &             -rol*xrl/xvisl**2*dvislt)
              dilw(mi)=prf*rgwm*rol*drlw/xvisl+prf*drgwm3*rol*xrl/xvisl
              dilm(mi)=dprf4*rgwm*rol*xrl/xvisl+prf*drgwm4*rol*xrl/xvisl
            endif
            dglp(mi)=drolp
            dgle(mi)=drolt
            enlfmeth(mi)=enl
...
c     modify flow terms for new upwind scheme
     ...
     &              -rov*xrv/xvisv**2*dvisvt)
              divw(mi)=prf*rgwm*rov*drvw/xvisv+prf*drgwm3*rov*xrv/xvisv
c              divm(mi)=dprf4*rov*xrv/xvisv
c              divw(mi)=prf*rgwm*rov*drvw/xvisv+prf*drgwm3*rov*xrv/xvisv
              divm(mi)=dprf4*rgwm*rov*xrv/xvisv+prf*drgwm4*rov*xrv/xvisv
            endif
            dgvp(mi)=drovp
            dgve(mi)=drovt
            envfmeth(mi)=env
...
c     form dissociation flow term
            skhyd(mi) = rathyd*skhyd_temp(mi)
c     form heat dissociation flow term (minus means add heat to system)
c don't need enthalpy rate as this is included in enthanlpy of materials
            qhhyd(mi) = 0.0d0
