...
c
      if(.not.allocated(dum_ani)) then
       ...
       allocate(dume_ani(neq))
       allocate(dumex_ani(neq))
       allocate(dumey_ani(neq))
       allocate(dumez_ani(neq))       
       allocate(it4a(200))
       allocate(it5a(200))
       allocate(it6a(200))
      endif
...
c     + x face
     ...
     &                   axyf*ddendekb*sumxg  
            dume_ani(iz) = dume_ani(iz) + dfxei
            dume_ani(kb_adv_x) = dume_ani(kb_adv_x) + dfxekb
            dumex_ani(iz) = dumex_ani(iz) - dfxei
            dumex_ani(kb_adv_x) = dumex_ani(kb_adv_x) - dfxekb
           endif
...
c     + y face
     ...
     &                   axyf*ddendpkb*sumyg
           ...
           if(irdof.ne.13) then
	               ...
	               daxyfei = fid1*dilei
	               daxyfekb = fid*dilekb            
	               dfyei = daxyfei*(sumy + dend*sumyg) +  
     &                  axyf*ddendei*sumyg
	               dfyekb = daxyfekb*(sumy + dend*sumyg) + 
     &                   axyf*ddendekb*sumyg  
	               dume_ani(iz) = dume_ani(iz) + dfyei
	               dume_ani(kb_adv_y) = dume_ani(kb_adv_y) + dfyekb
	               dumex_ani(iz) = dumex_ani(iz) - dfyei
	               dumex_ani(kb_adv_y) = dumex_ani(kb_adv_y) - dfyekb
           endif
     ...
     &                   axyf*ddendpkb*sumzg
	   ...
	   if(irdof.ne.13) then
	   	    dilekb=dile(kb_adv_z)
	            ...
	            daxyfei = fid1*dilei
	            daxyfekb = fid*dilekb            
	            dfzei = daxyfei*(sumz + dend*sumzg) +  
     &                axyf*ddendei*sumzg
	            dfzekb = daxyfekb*(sumz + dend*sumzg) + 
     &                axyf*ddendekb*sumzg  
	            dume_ani(iz) = dume_ani(iz) + dfzei
	            dume_ani(kb_adv_z) = dume_ani(kb_adv_z) + dfzekb
	            dumex_ani(iz) = dumex_ani(iz) - dfzei
	            dumex_ani(kb_adv_z) = dumex_ani(kb_adv_z) - dfzekb
           endif
