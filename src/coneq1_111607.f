      subroutine coneq1(ndummy, matnum, spec_num)  
!***********************************************************************
...
c
      ...
      parameter (tolvel = 1.d-30,toldisp = 1.d-20)
...
c                    sxzc=sx(iw,3)
                     if (ldsp.eq.0) then
                        ...
                        cord2y=cord(kz,2)
                        cord2z=cord(kz,3)
                        dispxavw = max(tclx(nsp,itrc(insp)),toldisp)
                        dispyavw = max(tcly(nsp,itrc(insp)),toldisp)
                        dispzavw = max(tclz(nsp,itrc(insp)),toldisp)
...
c_______GAZ___________11/16/2007_____________________________________

c                       pnxavg = 0.5*(pnxdum + pnx(n+kb))
c                       pnyavg = 0.5*(pnydum + pny(n+kb))
c	                  pnzavg = 0.5*(pnzdum + pnz(n+kb))
                        pnxavg = 2.0*pnxdum*pnx(n+kb)/
     +                    (pnxdum + pnx(n+kb)+tolvel)
                        pnyavg = 2.0*pnydum*pny(n+kb)/
     +					(pnydum + pny(n+kb)+tolvel)
	                  pnzavg = 2.0*pnzdum*pnz(n+kb)/
     +					(pnzdum + pnz(n+kb)+tolvel)
	                  pnxavg = max(abs(pnxavg),tolvel)
                        pnyavg = max(abs(pnyavg),tolvel)
	                  pnzavg = max(abs(pnzavg),tolvel)
	                  vmagavg = sqrt( pnxavg*pnxavg + pnyavg*pnyavg +
     +                  pnzavg*pnzavg)
     ...
     +                       ,newx,newy,newz)
C                        call rotate(cord(iz,1),cord(iz,2),cord(iz,3),
C     +                       pnxdum,pnydum,
C     +                       pnzdum,
C     +                       cord(kz,1),cord(kz,2),cord(kz,3)
C     +                       ,newx,newy,newz)
C_________________________________________________________________________
                        ...
                        cord2y=newy
                        cord2z=newz
                        dispxavw=max(tcly(nsp,itrc(insp)),toldisp)
                        dispyavw=max(tcly(nsp,itrc(insp)),toldisp)
                        dispzavw=max(tclx(nsp,itrc(insp)),toldisp)
                        tempx = (cord1x-cord2x)**2
                        tempy = (cord1y-cord2y)**2
                        tempz = (cord1z-cord2z)**2
                        templength=tempx+tempy+tempz
...
c                        slx2tt = sx2c*((vmag*alphaavw)+dum_bar)
                       slx2tt = sx2c*((vmagavg*alphaavw)+dum_bar)
c--------------------------------------------------------------------
                     endif
                     t5(neighc)=slx2tt
                  enddo
...
c_______GAZ___________11/16/2007_____________________________________
     ...
     +                       sin(thetav)*cord2xp+cos(thetav)*cord2yp
                        tempx = (cord1x-cord2x)**2
                        tempy = (cord1y-cord2y)**2
c                        dispxavw = tclx(nsp,itrc(insp))
c                        dispyavw = tcly(nsp,itrc(insp))
                        dispxavw=max(tclx(nsp,itrc(insp)),toldisp)
                        dispyavw=max(tcly(nsp,itrc(insp)),toldisp)
                        alphaavw = sqrt((dispxavw*dispyavw * 
     +                       (tempx+tempy))/
...
c----------------------------- 9/3/2004 -----------------------------
                        slx2tt = 
     &					  sx2c*(vmagavg*alphaavw+dum_bar + toldisp)
c--------------------------------------------------------------------
c_______GAZ___________11/16/2007_____________________________________
                     endif
                     t5(neighc)=slx2tt
...
c
     ...
     &                              'wvflux file prefix'
                   ...
                   open(99,file=fname)
                   rewind 99
                   write(99,*) 'cord(i,1) nodei nodekb wvflow(kg/s)
...
c----     --    - -  --  -    - -   -     -       -        -
                ...
                if(i.LT.kb) then
                write(99,*) cord(i,1), i, kb, a_wvxy(pntr),
