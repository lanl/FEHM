...
c     fully coupled but mat need to be enlarged in startup     
         ...
         if (idof_stress.ne.2.and.istrs.eq.2) then
            write (ierr, *) 'plain stress is not 2-D: stopping '
            if (iout .ne. 0) then
               write (iout,*)
...
c     if lithostatic then must use initcalc	   
     ...
     &              'initcalc set because lithostatic chosen'
...
c     plain strain
     ...
     &           ' NOTE macro "isot" not allowed with stress '    
            write(iptty,*) ' NOTE input changed', 
     &           '(thermal expansion now volume (=linear*3)' 
            write(iptty,*) ' NOTE input changed',
     &           '(Biot term now a in the term a*(1-K/Kb)'  
            write(iptty,*) ' NOTE ifinv=1', 
     &           '(finite volume disabled when stress cals present)'  
            write(iptty,*)'*********************************'       
         endif
         if (iout .ne. 0) then
            write(iout,*)'*********************************'
     ...
     &           ' NOTE macro "nobr" always set with stress (disabled)'
            write(iout,*)
     &           ' NOTE macro "isot" not allowed with stress '      
            write(iout,*) ' NOTE input changed ',
     &           '(thermal expansion now volume (=linear*3)' 
            write(iout,*) ' NOTE input changed ',
     &           '(Biot term now = a in the term a*(1-K/Kb)' 
            write(iout,*) ' NOTE ifinv=1 ',
     &           '(finite volume disabled when stress cals present)'
            write(iout,*)'*********************************'  
         end if      
         inobr = 1
...
c     call stress output subroutines
         ...
         if(icnl.ne.0) then            
c     ****   printout displacement information   ****
c     
     ...
     &              0.
            ...
            if(ntty.eq.2 .and. iout .ne. 0) then
               write(iout,*) ' ' 
               write(iout,*) 'displacements and forces ' 
               write(iout,6119) 
            end if
            if(tol_stress.ne.0.0d0) then 
               if(mlz.eq.-1) then
...
c     
            if(ntty.eq.2 .and. iout .ne. 0) then
               write(iout,*) ' ' 
               write(iout,*) 'Stresses (Convention: ',
     &              'Compression is Positive (sign)) ' 
               write(iout,7119)
            end if 
            if(iptty.gt.0) then
               write(iptty,*) ' ' 
               write(iptty,*) 'Stresses (Convention: ',
     &              'Compression is Positive (sign)) '  
               write(iptty,7119)
            end if 
            do   i=1,m
...
c     
 ...
 1200          format(1x,'node = ',i8,' xyz = ',1p,3g14.5,
     &              ' type = ',a5)	    
            enddo
            return
         endif
