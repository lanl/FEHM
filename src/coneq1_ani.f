      subroutine coneq1_ani(ndummy, matnum, spec_num)
!***********************************************************************
...
c
      ...
      real*8 cord1z,cord2z,newx,newy,newz,newdiff,concadiff,satr
      ...
      character*10 fsuffix
      integer iroot, isuffix
      logical found_name
...
c         nrhs_sol(1)=nrhs(2)
      ...
      do i=strindex,endindex
         ...
         if (irdof .ne. 13 .or. abs(ifree) .eq. 1) then
            satr = s(i)
         else
            satr = 1.d0
         end if
         ...
         do jm=jmi+1,ii2
            iq=iq+1
c            it10(iq)=istrw(jm-neqp1)
c            if (sx(it10(iq),isox)+sx(it10(iq),isoy)+sx(it10(iq),isoz)
c     &           .eq.0.) then
c               iq=iq-1
c            else
               it8(iq)=nelm(jm)+icd
...
c------- PHS ---------- 9/3/2004 -----------------------------
     ...
     &                    diffmfl(nsp,itrc(kbnsp)),ps(kb),satr,
     &                    phi(kb),t(kb))
               
                     dumkb = satr*newdiffkb*ps(kb)
...
c------- PHS ---------- 9/3/2004 -----------------------------
                     ...
                     neighc=it9(jm)
c                     iw=it10(jm)
c                     sx2c=sx(iw,isox)+sx(iw,isoy)+sx(iw,isoz)
                     sx2c=0.0d00
c                    sx3c=sx(iw,2)
c                    sxzc=sx(iw,3)
                     if (ldsp.eq.0) then
...
c                     sx2c=radkb*(sx(iw,isox)+sx(iw,isoy))
                     sx2c=0.0d00
...
c----------------- PHS ------- 9/3/2004 -----------------------------
     ...
     &                    diffmfv(nsp,itrc(kbnsp)),ps(kb),satr,
     &                    phi(kb),t(kb))
...
c
     ...
     &                     nmfil(5) .ne. ' ') then
                         ...
                         isuffix = len_trim(nmfil(5))
                         root(1:iroot) = nmfil(5)(1:iroot)
                         fsuffix(1:isuffix-iroot) = 
     &                        nmfil(5)(iroot+1:isuffix)
                      else
                         if (nmfil(2)(1:1) .eq. ' ' ) then
                            write (ierr, *) 
     &                           'FILE ERROR: nmfil2 file: ', nmfil(2),
     .                       ' unable to determine wvflux file prefix'
                            stop
                         else
                            call file_prefix(nmfil(2), iroot)
                            if (iroot .gt. 100) iroot = 100
                            isuffix = len_trim(nmfil(2))
                            root(1:iroot) = nmfil(2)(1:iroot)
                            fsuffix(1:isuffix-iroot) = 
     &                           nmfil(2)(iroot+1:isuffix)
                         end if
                      end if                   
                      fname =  root(1:iroot) // '_wvflux' 
                      fname = fname(1:iroot+7)//fsuffix(1:isuffix-iroot)
                      found_name = .true.
                   end if
                   open(99,file=fname)
                   rewind 99
