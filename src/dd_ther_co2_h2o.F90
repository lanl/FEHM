#ifdef DDFLAG
      subroutine dd_ther_co2_h2o(iflg,ndummy)
!***********************************************************************
...
!**********************************************************************
      ...
      PetscInt ndummy,mid,mi,ieosd,kq,icesd
      PetscReal dtin,dporpl,dportl
      PetscReal tl,pl,dpsatt
c      PetscReal enw,enl
c      PetscReal dhlt,dhlp
c      PetscReal rol
c      PetscReal drolp,drolt
      PetscReal xvisl
c      PetscReal dvislp
c      PetscReal dvislt
c      PetscReal env
c      PetscReal dhvt,dhvp
c      PetscReal rov
      PetscReal xvisv
      PetscReal qdis,dtd
      PetscReal cp,por,vol,pldif,eskd,eqdum
      PetscReal damp,daep,daep1,damh,daeh
      PetscReal dql,hprod,dhprdp,dhprde,htc,tbound,hflux
      PetscReal dhflxp,sbound,cprd,edif,denrd
      PetscReal dpldt,daep2
      PetscReal dqv,dhflxe,dtps,permsd1
      PetscReal dhflxem,dhflxpm,daeh1,daeh2,pcpww
      PetscReal, allocatable :: sto1(:)

      PetscReal psatd
      PetscReal dum1,dum2,dum3,dum5,dum6
      PetscReal damw,daew,dhprdw                        
      PetscReal dhflxew
      PetscReal ensc,denscp,densct,rosc,droscp,drosct,xvisc,dviscp,dvisct
      PetscReal dtol
...
c     will over-ride above co2 production
      PetscReal fracwmin
      PetscReal value(9),dumb
      
c      PetscReal drolyc, drolya, roa, droadp, droadt, ena, denadt,denadp
      PetscReal   ycp, dycp
c      PetscReal  rlw, drlww, drlwg, drlwp, drlwt
c      PetscReal rll, drllw, drllg, drllp, drllt
c      PetscReal drolw, denlt, denlp, denlw, denlya, denlyc
c      PetscReal visl, dvislya, dvislyc, dvislw
c      PetscReal row, drowp, drowt, drowya, drowyc, denwp, denwt
c      PetscReal visw, dviswp, dviswt
c      PetscReal dprmp, dprmt, dprmw, dprmyc, dprmya
c      PetscReal damyc, damya, daeyc, daeya
c      PetscReal dhprdyc, dhprdya, rlv, drlvw, drlvg, drlvp, drlvt
c      PetscReal drovw, drovya, drovyc, denvw, denvya, denvyc, visv, dvisvya
c      PetscReal dvisvyc, dvisvw, demwyc, denwyc, denwya
c      PetscReal vpartial,dvpardt
c      PetscReal xs, dxsw, dxsg, denwfw, denwfg, s1, s2, ds1dw, ds1dg
c new variables
c      PetscReal enx,denxp,denxe,denxyc,denxya, vis_tol
      PetscReal :: cden_correction, mol
c      PetscReal :: permsd11 = 0., dprmp1 = 0., dprmt1 = 0., dprmw1 = 0.
c      PetscReal :: permsd12 = 0., dprmp2 = 0., dprmt2 = 0., dprmw2 = 0.
c      PetscReal :: permsd13 = 0., dprmp3 = 0., dprmt3 = 0., dprmw3 = 0.
...
c     
         ...
         if(ico2prop_flg.eq.1) then
            wat_prop(mi) = row
            wat_prop(dd_dd_neq+mi) = drowp
            wat_prop(2*dd_dd_neq+mi) = drowt
            wat_prop(3*dd_dd_neq+mi) = 0.d0
            wat_prop(4*dd_dd_neq+mi) = 0.d0
            wat_prop(5*dd_dd_neq+mi) = enw
            wat_prop(6*dd_dd_neq+mi) = denwp
            wat_prop(7*dd_dd_neq+mi) = denwt
            wat_prop(8*dd_dd_neq+mi) = visw
            wat_prop(9*dd_dd_neq+mi) = dviswp
            wat_prop(10*dd_dd_neq+mi) = dviswt
            call co2_properties(1,duma,pl,tl,dum1,icesd,dumb,value,
     &           duma)
            if(icesd.eq.3) then
               co2_prop(9*dd_dd_neq+mi) = rosc
               co2_prop(10*dd_dd_neq+mi) = drosct
               co2_prop(11*dd_dd_neq+mi) = droscp
               co2_prop(12*dd_dd_neq+mi) = ensc
               co2_prop(13*dd_dd_neq+mi) = densct
               co2_prop(14*dd_dd_neq+mi) = denscp
               co2_prop(15*dd_dd_neq+mi) = xvisc
               co2_prop(16*dd_dd_neq+mi) = dvisct
               co2_prop(17*dd_dd_neq+mi) = dviscp
            else
               co2_prop(mi) = rosc
               co2_prop(dd_dd_neq+mi) = drosct
               co2_prop(2*dd_dd_neq+mi) = droscp
               co2_prop(3*dd_dd_neq+mi) = ensc
               co2_prop(4*dd_dd_neq+mi) = densct
               co2_prop(5*dd_dd_neq+mi) = denscp
               co2_prop(6*dd_dd_neq+mi) = xvisc
               co2_prop(7*dd_dd_neq+mi) = dvisct
               co2_prop(8*dd_dd_neq+mi) = dviscp
            endif
...
c     drol_dt=drol_dt+drol_dyc*dmol(mi+dd_dd_neq)
c     endif
               drol_dya = 0.0
               rol = rol_h2o/rol_d
...
c Hari
            rolf(mi) = rol
            wat_prop(dd_dd_neq+mi) = drolp
            wat_prop(2*dd_dd_neq+mi) = drolt
            wat_prop(3*dd_dd_neq+mi) = drolyc
            wat_prop(4*dd_dd_neq+mi) = drolya
            wat_prop(5*dd_dd_neq+mi) = enl
            wat_prop(6*dd_dd_neq+mi) = dhlp
            wat_prop(7*dd_dd_neq+mi) = dhlt
            wat_prop(8*dd_dd_neq+mi) = xvisl
            wat_prop(9*dd_dd_neq+mi) = dvislp
            wat_prop(10*dd_dd_neq+mi) = dvislt
c     
c     air properties
c     
...
c     dissolved CO2 mass fraction for two phase
               dmol(mi) = dmol(mi)+dmol(mi+dd_dd_neq)*dtps
               dmol(mi+dd_dd_neq) = 0.d0
c     tco2(mi) = tl
            endif
     ...
     &              value,duma)
               if(icesd.eq.3) then
                  co2_prop(9*dd_neq+mi) = value(1)
                  co2_prop(10*dd_neq+mi) = value(2)
                  co2_prop(11*dd_neq+mi) = value(3)
                  co2_prop(12*dd_neq+mi) = value(4)
                  co2_prop(13*dd_neq+mi) = value(5)
                  co2_prop(14*dd_neq+mi) = value(6)
                  co2_prop(15*dd_neq+mi) = value(7)
                  co2_prop(16*dd_neq+mi) = value(8)
                  co2_prop(17*dd_neq+mi) = value(9)
               else
                  co2_prop(mi) = value(1)
                  co2_prop(dd_neq+mi) = value(2)
                  co2_prop(2*dd_neq+mi) = value(3)
                  co2_prop(3*dd_neq+mi) = value(4)
                  co2_prop(4*dd_neq+mi) = value(5)
                  co2_prop(5*dd_neq+mi) = value(6)
                  co2_prop(6*dd_neq+mi) = value(7)
                  co2_prop(7*dd_neq+mi) = value(8)
                  co2_prop(8*dd_neq+mi) = value(9)
               endif
               dtpaco2(mi)=0.d0
               dtpaeco2(mi)=1.d0
c     RJP 03/02/08 added mixture enthalpy for water. added the enthalpy derivative wrt yc and ya
c     to wat_prop array. As the value of wat_prop(5*dd_neq+mi) is getting modified, the derivative
c     is calculated first.
               wat_prop(11*dd_neq+mi) = value(4)-wat_prop(5*dd_neq+mi)
               wat_prop(12*dd_neq+mi) = -wat_prop(5*dd_neq+mi)
               wat_prop(5*dd_neq+mi) = ywat*wat_prop(5*dd_neq+mi)+yco2*
     &              value(4)
               wat_prop(6*dd_neq+mi) = ywat*wat_prop(6*dd_neq+mi)+yco2*
     &              value(6)
               wat_prop(7*dd_neq+mi) = ywat*wat_prop(7*dd_neq+mi)+yco2*
     &              value(5)
c     wat_prop(11*dd_neq+mi) = 0.d0
c     wat_prop(12*dd_neq+mi) = 0.d0
            else
...
c     are part of the gas phase.
     ...
     &              value,duma)
               co2_prop(mi) = value(1)
               co2_prop(dd_neq+mi) = 0.d0
               co2_prop(2*dd_neq+mi) = value(3)
               co2_prop(3*dd_neq+mi) = value(4)
               co2_prop(4*dd_neq+mi) = 0.d0
               co2_prop(5*dd_neq+mi) = value(6)
               co2_prop(6*dd_neq+mi) = value(7)
               co2_prop(7*dd_neq+mi) = 0.d0
               co2_prop(8*dd_neq+mi) = value(9)
...
C     RJP 04/08/07 assumed density of co2-rich phase is independent of mixed water & air.
               co2_prop(9*dd_neq+mi) = value(1)
               co2_prop(10*dd_neq+mi) = 0.d0
               co2_prop(11*dd_neq+mi) = value(3)
               co2_prop(12*dd_neq+mi) = value(4)
               co2_prop(13*dd_neq+mi) = 0.d0
               co2_prop(14*dd_neq+mi) = value(6)
               co2_prop(15*dd_neq+mi) = value(7)
               co2_prop(16*dd_neq+mi)=0.d0
               co2_prop(17*dd_neq+mi)=value(9)	
c     RJP 03/02/08 added mixture enthalpy for water
               wat_prop(11*dd_neq+mi) = value(4)-wat_prop(5*dd_neq+mi)
               wat_prop(12*dd_neq+mi) = -wat_prop(5*dd_neq+mi)
               wat_prop(13*dd_neq+mi) = dxsg*yco2*
     &              (value(4)-co2_prop(3*dd_neq+mi))
               wat_prop(14*dd_neq+mi) = dxsw*yco2*
     &              (value(4)-co2_prop(3*dd_neq+mi))
               wat_prop(5*dd_neq+mi) = ywat*wat_prop(5*dd_neq+mi)+yco2*
     &              (value(4)*xs+(1.d0-xs)*co2_prop(3*dd_neq+mi))
               wat_prop(6*dd_neq+mi) = ywat*wat_prop(6*dd_neq+mi)+yco2*
     &              (value(6)*xs+(1.d0-xs)*co2_prop(5*dd_neq+mi))
               wat_prop(7*dd_neq+mi) = ywat*wat_prop(7*dd_neq+mi)+yco2*0.d0
               wat_prop(dd_neq+mi) = wat_prop(dd_neq+mi)+
     &              wat_prop(2*dd_neq+mi)*dtpsc(mi)
               wat_prop(2*dd_neq+mi) = 0.d0
               wat_prop(6*dd_neq+mi) = wat_prop(6*dd_neq+mi)+
     &              wat_prop(7*dd_neq+mi)*dtpsc(mi)
               wat_prop(7*dd_neq+mi) = 0.d0
               wat_prop(9*dd_neq+mi) = wat_prop(9*dd_neq+mi)+
     &              wat_prop(10*dd_neq+mi)*dtpsc(mi)
               wat_prop(10*dd_neq+mi) = 0.d0
               dtpaco2(mi)=dtpsc(mi)
               dtpaeco2(mi)=0.d0
...
c     
            if((icesd.eq.1).or.(icesd.eq.4))then 
               rol = co2_prop(mi)
               drolt=co2_prop(dd_neq+mi)
               drolp=co2_prop(2*dd_neq+mi)
               drolw=0.d0
               drolya=0.d0
               drolyc=0.d0
               enl=co2_prop(3*dd_neq+mi)
               denlt=co2_prop(4*dd_neq+mi)
               denlp=co2_prop(5*dd_neq+mi)
               denlw=0.d0
               denlya=0.d0
               denlyc=0.d0
               visl=co2_prop(6*dd_neq+mi) + vis_tol
               dvislt=co2_prop(7*dd_neq+mi)
               dvislp=co2_prop(8*dd_neq+mi)
               dvislya=0.d0
               dvislyc=0.d0
               dvislw=0.d0
            else
               rol = co2_prop(9*dd_neq+mi)
               drolt=co2_prop(10*dd_neq+mi)
               drolp=co2_prop(11*dd_neq+mi)
               drolw=0.d0
               drolya=0.d0
               drolyc=0.d0
               enl=co2_prop(12*dd_neq+mi)
               denlt=co2_prop(13*dd_neq+mi)
               denlp=co2_prop(14*dd_neq+mi)
               denlw=0.d0
               denlya=0.d0
               denlyc=0.d0
               visl=co2_prop(15*dd_neq+mi)+ vis_tol
               dvislt=co2_prop(16*dd_neq+mi)
               dvislp=co2_prop(17*dd_neq+mi)
               dvislya=0.d0
               dvislyc=0.d0
               dvislw=0.d0
            endif				
            row = wat_prop(mi)
            drowp=wat_prop(dd_neq+mi)
            drowt=wat_prop(2*dd_neq+mi)
            drowyc=wat_prop(3*dd_neq+mi)
            drowya=wat_prop(4*dd_neq+mi)	
            enw=wat_prop(5*dd_neq+mi)
            denwp=wat_prop(6*dd_neq+mi)
            denwt=wat_prop(7*dd_neq+mi)
            visw=wat_prop(8*dd_neq+mi) + vis_tol
            dviswp=wat_prop(9*dd_neq+mi)
            dviswt=wat_prop(10*dd_neq+mi)
            denwyc=wat_prop(11*dd_neq+mi)
            denwya=wat_prop(12*dd_neq+mi)
            denwfg=wat_prop(13*dd_neq+mi)
            denwfw=wat_prop(14*dd_neq+mi)
...
c           else
     ...
     &                       +dqyc(mi)*dmol(mi+dd_neq)
                     else
                        dq(mi)=dq(mi)+dqyc(mi)*dmol(mi)
                        dqt(mi)=dqt(mi)+dqyc(mi)*dmol(mi+dd_neq)
                     endif
                  endif
               endif
     ...
     &                         +dqhyc(mi)*dmol(mi+dd_neq)
	                    else
							dqh(mi)=dqh(mi)+dqhyc(mi)*dmol(mi)
							deqh(mi)=deqh(mi)+dqhyc(mi)*dmol(mi+dd_neq)
						endif
					endif
                  endif
			endif
...
c     store derivatives of accumulation terms
     ...
     &                       damyc*dmol(mi+dd_neq)
                        depf(mi)=depf(mi)+daeyc*dmol(mi)+
     &                       daeyc*dmol(mi+dd_neq)
                     else
                        dmpf(mi)=dmpf(mi)+damyc*dmol(mi)
                        dmef(mi)=dmef(mi)+damyc*dmol(mi+dd_neq)
                        depf(mi)=depf(mi)+daeyc*dmol(mi)
                        deef(mi)=deef(mi)+daeyc*dmol(mi+dd_neq)
                     endif
                  endif
               endif
               t(mi)=tl
...
c     
     ...
     &                       +diwyc(mi)*dmol(mi+dd_neq)
                        divp(mi)=divp(mi)+divyc(mi)*dmol(mi)
     &                       +divyc(mi)*dmol(mi+dd_neq)
                     else
                        diwp(mi)=diwp(mi)+diwyc(mi)*dmol(mi)
                        diwe(mi)=diwe(mi)+diwyc(mi)*dmol(mi+dd_neq)
                        divp(mi)=divp(mi)+divyc(mi)*dmol(mi)
                        dive(mi)=dive(mi)+divyc(mi)*dmol(mi+dd_neq)
                     endif
                  endif
               endif
...
c     endif
         ...
         do     mid=1,dd_neq
            ...
            denei(mi)=(sto1(mi+dd_neq)-deneh(mi))*dtin
         enddo    
...
c     
            ...
            rov = co2_prop(9*dd_neq+mi)
            drovt=co2_prop(10*dd_neq+mi)
            drovp=co2_prop(11*dd_neq+mi)
            drovw=0.d0
            drovya=0.d0
            drovyc=0.d0
            env=co2_prop(12*dd_neq+mi)
            denvt=co2_prop(13*dd_neq+mi)
            denvp=co2_prop(14*dd_neq+mi)
            denvw=0.d0
            denvya=0.d0
            denvyc=0.d0
            visv=co2_prop(15*dd_neq+mi) + vis_tol
            dvisvt=co2_prop(16*dd_neq+mi)
            dvisvp=co2_prop(17*dd_neq+mi)
            dvisvya=0.d0
            dvisvyc=0.d0
            dvisvw=0.d0
...
c     transport in CO2-rich phase.
     ...
     &                       +divyc(mi)*dmol(mi+dd_neq)
                        ...
                        diwe(mi)=diwe(mi)+diwyc(mi)*dmol(mi+dd_neq)
                        dilp(mi)=dilp(mi)+dilyc(mi)*dmol(mi)
                        dile(mi)=dile(mi)+dilyc(mi)*dmol(mi+dd_neq)
                        divp(mi)=divp(mi)+divyc(mi)*dmol(mi)
                        dive(mi)=dive(mi)+divyc(mi)*dmol(mi+dd_neq)
                     endif
                  endif
...
c     endif
         ...
         do mid=1,dd_neq
            ...
            deneco2i(mi)=(sto1(mi+dd_neq)-deneco2h(mi))*dtin
         enddo    
