...
c               call plc1_test(1,days,jplc)
               if ( icns(nsp) .gt. -1 ) then
c                  call plc1_test(0,days,jplc)
...
c gaz_060925
      subroutine plc1_test(iflg,days,jplc)
c
c write out tracer 
c 
      use comai, only : ldna
      ...
      use comflow
      implicit none
      integer iflg,iplc,jplc,iaxy,i
      integer open_file
      real*8 days,days_test,flow_test
      character*18 plc_tst_base, a_axy_tst_base
      character*4 base
      logical exist,exists
      parameter(days_test = 350.00,flow_test= 1.d-12)
      if(iflg.eq.0) then
       plc_tst_base = 'plc_anv_0000000000'
c create file
       write(plc_tst_base(9:18),'(f10.2)') days              
        do i =9,18
         if(plc_tst_base(i:i).eq.' ') plc_tst_base(i:i) = '0'
        enddo
c
      inquire(file = plc_tst_base, exist = exists)
       if(.not.exists) then
        iplc = open_file(plc_tst_base,'unknown')
        write(iplc,53)  'sptr generated tracer plotc1' 
        jplc =0
        do i = 1,n0
         if(abs(an(i)).lt.1.d-30) jplc = jplc + 1
        enddo
        write(iplc,54) days, n0-jplc
        write(iplc,55) (an(i) , i=1,n0 )
       else
        iplc = open_file(plc_tst_base,'old')
        write(iplc,53)  'sptr generated tracer plotc1' 
        jplc =0
        do i = 1,n0
         if(abs(an(i)).lt.1.d-30) jplc = jplc + 1
       ...
       if(days.gt.1000.)then
        write(iplc,*) 'days gt 1000, stopping'
        close(iplc)
        stop
       else
        close(iplc)
...
c output a_axy
       if(days.gt.days_test) return
       a_axy_tst_base  =  'a_axy_tst_000'
       inquire(file = a_axy_tst_base, exist = exists)
       if(.not.exists) then
        iaxy = open_file(a_axy_tst_base,'unknown')
        write(iaxy,53)  'flow field from plotc1' 
        jplc = 0
        do i = 1,ldna
         if(abs(a_axy(i)).lt.flow_test) jplc = jplc + 1
        enddo
        write(iaxy,54) days, ldna-jplc
        write(iaxy,55) (a_axy(i) , i=1,ldna )
       else
        iaxy = open_file(a_axy_tst_base,'old')
        write(iaxy,53)  'flow field from plotc1' 
        jplc = 0
        do i = 1,ldna
         if(abs(a_axy(i)).lt.flow_test) jplc = jplc + 1
        enddo
        write(iaxy,54) days,flow_test, ldna-jplc
        write(iaxy,55) (a_axy(i) , i=1,ldna )
       endif
      else if(iflg.eq.2) then
      endif
53    format (1x,a)
54    format ('days ',f15.6,' ft ',1p,g10.3,' values gt flow_test ',i9)
55    format (1x,1p,5(g14.7))
      return
      end  subroutine plc1_test