      subroutine sub_bcgstabnn(n,a,bb,b,na,nb,nrhs,ncon,nop,l,work,tol
...
!D2 
!D2 $Log:   /pvcs.config/fehm90/src/sub_bcgstabnn.f_a  $ 
!D2 
!D2    Rev 2.5   06 Jan 2004 10:44:04   pvcs
!D2 FEHM Version 2.21, STN 10086-2.21-00, Qualified October 2003
!D2 
!D2    Rev 2.4   29 Jan 2003 09:19:18   pvcs
...
cM      enddo
      ...
      call constant_value(x,zero,n,idof,nrhs_dum)
      call solven(n,bb,nb,nop,irb,iirb,npvt,piv,nrhs_dum,nrhs_dum,idof,
     *           work(1,r),rw)
...
c
	...
	call equal_array(work(1,xp),x,n,idof,nrhs_dum,nrhs_dum)
	call constant_value(x,zero,n,idof,nrhs_dum)
...
c
	 rho0 = -omega*rho0
	 do k=1,l
	    rho1 = ddot (nn, work(1,rr), 1, work(1,r+k-1), 1)
	    ...
	    do j=0,k-1
		  ...
		  work(i,u+j) = work(i,r+j) - beta*work(i,u+j)
	       enddo
	    enddo
	    call mvn(n,idof,a,na,ncon,work(1,u+k-1),work(1,u+k),
     *               nrhs_dum,nrhs_dum)
	    call solven(n,bb,nb,nop,irb,iirb,npvt,piv,nrhs_dum,
     *                 nrhs_dum,idof,work(1,u+k),rw)
	    nmv = nmv+1
	    sigma = ddot (nn, work(1,rr), 1, work(1,u+k), 1)
	    if (sigma.eq.zero) then
	       info = 2
		goto 5000
	    endif
	    alpha = rho1/sigma
	    call daxpy (nn, alpha, work(1,u), 1, x, 1)
	    do j=0,k-1
	       call daxpy (nn, (-alpha), work(1,u+j+1), 1,
     $              work(1,r+j), 1)
	    enddo
	    call mvn(n,idof,a,na,ncon,work(1,r+k-1),work(1,r+k),
     *               nrhs_dum,nrhs_dum)
	    call solven(n,bb,nb,nop,irb,iirb,npvt,piv,nrhs_dum,
     *                 nrhs_dum,idof,work(1,r+k),rw)
	    nmv = nmv+1
cM	    rnrm = dnrm2 (nn, work(1,r), 1)
...
c
	 do i=1,l+1
	    call dgemv ('t', nn, l+1-(i-1), one, work(1,r+i-1),
     $           nn, work(1,r+i-1), 1, zero, rwork(i,z+i-1), 1)
            if(l-(i-1).ne.0) then
...
c
     ...
     $        rwork(1,y0), 1, zero, rwork(1,y), 1)
         kappa0 = sqrt(abs(ddot (l+1, rwork(1,y0), 1,
     $        rwork(1,y), 1)))

	 call dsymv ('u', l+1, one, rwork(1,z), l+1,
     $        rwork(1,yl), 1, zero, rwork(1,y), 1)
	 kappal = sqrt(abs(ddot (l+1, rwork(1,yl), 1,
     $        rwork(1,y), 1)))
...
c
	 ...
	 call dgemv ('n', nn, l, (-one), work(1,u+1), nn,
     $        rwork(2,y0), 1, one, work(1,u), 1)
	 call dgemv ('n', nn, l, one, work(1,r), nn,
     $        rwork(2,y0), 1, one, x, 1)
	 call dgemv ('n', nn, l, (-one), work(1,r+1), nn,
...
c
     ...
     $        .or.xpdt)
	 if (rcmp) then
	    call mvn(n,idof,a,na,ncon,x,work(1,r),nrhs_dum,nrhs_dum)
	    call solven(n,bb,nb,nop,irb,iirb,npvt,piv,nrhs_dum,
     *           nrhs_dum,idof,work(1,r),rw)
            nmv = nmv + 1
	    do i=1,nn
...
cM	       call dlaset ('n', nn, 1, zero, zero, x, 1)
	call constant_value(x,zero,n,idof,nrhs_dum)
cM	       call dcopy (nn, work(1,r), 1, work(1,bp), 1)
	call equal_array(work(1,bp),work(1,r),n,idof,nrhs_dum,nrhs_dum)
	       mxnrmx = rnrm
	    endif
	 endif
...
cM      enddo
cM      call solve(n,bb,nop,irb,iirb,npvt,piv,nrhs_dum,nrhs_dum,idof,
cM     *  work(1,r),rw)
...
c     tol = rnrm/rnrm0
 ...
 5000 call equal_array(b,x,n,idof,nrhs,nrhs_dum)
      call equal_array(rw,work(1,r),n,idof,nrhs_dum,nrhs_dum)
