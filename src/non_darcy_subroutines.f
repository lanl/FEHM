...
c gaz 071025
      real*8 dhvpc_i,drovpc_i,dvvpc_i
      real*8 xnv_i,dxnvp_i,dxnvt_i,dxnvpc_i
      real*8 dhlpc_i,drolpc_i,dvlpc_i
      real*8 xnl_i,dxnlp_i,dxnlt_i,dxnlpc_i  
      
      real*8 dhvpc_j,drovpc_j,dvvpc_j
      real*8 xnv_j,dxnvp_j,dxnvt_j,dxnvpc_j
      real*8 dhlpc_j,drolpc_j,dvlpc_j
      real*8 xnl_j,dxnlp_j,dxnlt_j,dxnlpc_j  
c 
      real*8  fid, fid1     
      real*8 xvisl_temp
c gaz 050425
...
c gaz 061025 new approach
c since called after thrmwc, fluid param, phase state 
c and deriv calculated
c vapor
      env_i    =	envf(i) 
      dhvp_i   =	devf(i) 
      dhvt_i   =	devef(i)
      ...
      drovpc_i =	dgvc(i) 
      xvisv_i  =    xvisv_nd(i) 	
      dvisvp_i =    dvislp_nd(i)	
      dvisvt_i =    dvislt_nd(i)	
      dvvpc_i  =    dvislc_nd(i)	
      ...
      drovpc_j =	dgvc(j) 
      xvisv_j  =    xvisv_nd(j)    
      dvisvp_j =    dvisvp_nd(j)  
      dvisvt_j =    dvisvt_nd(j)  
      dvvpc_j  =    dvisvc_nd(j)  
...
c liquid 
      ...
      drolpc_i =	dglc(i) 
      xvisl_i  =    xvisl_nd(i) 	
      dvislp_i =    dvislp_nd(i)	
      dvislt_i =    dvislt_nd(i)	
      dvlpc_i  =    dvislc_nd(i)	
      ...
      drolpc_j =	dglc(j) 
      xvisl_j  =    xvisl_nd(j)   
      dvislp_j =    dvislp_nd(j)  
      dvislt_j =    dvislt_nd(j)  
      dvlpc_j  =    dvislc_nd(j)  
      ...
      dxnlt_j  =	dclef(j)  
      dxnlpc_j = dclcf(j) 
c weighted parameters and deivatives  
        muvij = xvisv_j*fid + xvisv_i*fid1
        dmuvijpi =  dvisvp_i*fid1
        dmuvijpj =  dvisvp_j*fid
        dmuvijei =  dvisvt_i*fid1
        dmuvijej =  dvisvt_j*fid  
        dmuvijci =  dvvpc_i*fid1
        dmuvijcj =  dvvpc_j*fid        
        rolij = rol_j*fid + rol_i*fid1
        drolijpi =  drolp_i*fid1
        drolijpj =  drolp_j*fid
        drolijei =  drolt_i*fid1
        drolijej =  drolt_j*fid  
        drolijci =  drolpc_i*fid1
        drolijcj =  drolpc_j*fid        
        enlij = enl_j*fid + enl_i*fid1
        denlijpi =  dhlp_i*fid1
        denlijpj =  dhlp_j*fid
        denlijei =  dhlt_i*fid1
        denlijej =  dhlt_j*fid
        denlijci =  dhlpc_i*fid1
        denlijcj =  dhlpc_j*fid
        muij = xvisl_j*fid + xvisl_i*fid1
        dmuijpi =  dvislp_i*fid1
        dmuijpj =  dvislp_j*fid
        dmuijei =  dvislt_i*fid1
        dmuijej =  dvislt_j*fid  
        dmuijci =  dvlpc_i*fid1
        dmuijcj =  dvlpc_j*fid    
        rovij = rov_j*fid + rov_i*fid1
        drovijpi =  drovp_i*fid1
        drovijpj =  drovp_j*fid
        drovijei =  drovt_i*fid1
        drovijej =  drovt_j*fid 
        drovijei =  drovpc_i*fid1
        drovijej =  drovpc_j*fid          
        envij = env_j*fid + env_i*fid1
        denvijpi =  dhvp_i*fid1
        denvijpj =  dhvp_j*fid
        denvijei =  dhvt_i*fid1
        denvijej =  dhvt_j*fid
        denvijci =  dhvpc_i*fid1
        denvijcj =  dhvpc_j*fid         
       endif 
       endif
      return 
      end
