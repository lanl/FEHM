...
C***********************************************************************
      ...
      PetscInt nmatd, index(6)
      PetscInt i, j, icoupl_iter, id, idl, idofm 
      PetscInt jj,ii,idiag
      PetscInt iprint,inorm
      PetscInt neqp1, nrhs1, nrhs2, nrhs3, nsizea, nsizea1 
      PetscReal, allocatable :: dumz(:)
      PetscReal, allocatable :: dumn(:)
      PetscReal, allocatable :: sto5(:)   
      PetscReal  facr, fdum2, tollr, tolls
      PetscReal tmch1
...
c
      ...
      call DivideVariableArray(phi_co2, unstructured_grid,face_vertices,n_ss_faces, &
...
c     
      ...
      enddo
         ...
         call dd_ther_co2_h2o(3,0)

c
      call dd_co2h2o_combine(1,idofm)
c      
...
c RJP 04/30/05 added following for zero porosity nodes
      ...
      if(islord.ne.0) call switchb(bp,nrhs,nrhsb,islord,idofm,1,dd_neq)
...
c  gaz 2-27-03
       if(inorm.ne.0) then
        call normal_dof(dd_neq,a,bp,nelm,nmat,nrhs,nelmdg
...
c
         if(gdpm_flag.eq.0) then 
            if (igauss .gt. 1) then
!               call solve_new(dd_neq,A_mat,B_mat,BP_mat,nmat,nb,nrhs,nelm,nop,north
!     &              ,tollr,irb,iirb,npvt,gmres,dumz,piv
!     &              ,h,c,ss,g,y,iter,iback,idofm,iptty,maxor,accm)
				
				call petsc_solver(A_mat,BP_mat,nmat,nrhs,nelm,tollr)       ! petsc solver
            else
!               call solve_new(dd_neq,A_mat,B_mat,BP_mat,nmat,nrhs,nelm,nelm
...
c   enforce thermal equilibrium( set t2 = t1)
	...
	enddo
      call dd_co2h2o_combine(2,idofm)
