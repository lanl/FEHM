      subroutine geneq2_wellphysics(i)
!***********************************************************************
...
c     real*8 sxzc
      ...
      real*8 grav_air
      real*8 dlp, dis, vavg, dlvi, dlvkb, denavg, dilavg
      real*8 daxypi, daxypkb, daxyvi, daxyvkb        
...
c
            do 60 jm=1,iq
               ...
               neighc=it9(jm)
               sx4d=t6(neighc)
               t8(neighc)=phi(kb)+0.5*sx4d*(rolf(i)+rolf(kb))
     *              *(cord(kz,igrav)-cord(iz,igrav))
               t10(neighc) = 0.5*(vsupl(i)+vsupl(kb))
 60         continue
c
...
c
            ...
            do 61 jm=1,iq
               ...
               kz=kb-icd
               neighc=it9(jm)
c upind based on average superficial velocity
                  fid=0.5
                  axyd=t10(neighc)
...
c
            if(isl.ne.0) then
               do 62 jm=1,iq
                  ...
                  axyd=t8(neighc)
                  vavg = t10(neighc)
                  dis = (cord(kz,igrav)-cord(iz,igrav))
c dil contains the dynamic resistance term (vis/k for darcy flow)               
                  dilkb=dil(kb)
                  dilpkb=dilp(kb)             
c pressure equation
                  dilavg=(fid*dilkb+fid1*dili)
                  dlp = phi(kb) - axyd - dilavg*vavg*dis            
                  dlpi= -0.5*sx4d*dglp(i)*dis - fid1*dili*vavg*dis
                  dlpkb= 1.0 - 0.5*sx4d*dglp(kb)*dis - 
     &              fid1*dili*vavg*dis
                  dlvi = 0.5*dilavg*dis
                  dlvkb = 0.5*dilavg*dis
c velocities dont change sign (must correct) leaving node (+)
                  denavg = (fid*rolf(kb)+fid1*rolf(i))
                  axy = denavg*vavg
                  daxypi = fid1*dglp(i)*vavg
                  daxypkb = fid*dglp(kb)*vavg
                  daxyvi = denavg*0.5
                  daxyvkb = denavg*0.5
c     
