...
c gaz 030224 added isothermal diffusion
            if(idiff_iso.ne.0) then
             thxkb=thx(kb)
             thykb=thy(kb)
             thzkb=thz(kb)
...
c gaz 030224 corrected thx access violation
            if(idiff_iso.ne.0) then
             thxkb=thx(kb)
             thykb=thy(kb)
             sx2t=2.*thxi*thxkb/(thxi+thxkb)
...
c
      ...
      do 66 jm=1,iq
         ...
         dclkb=dclf(kb)*ctest
         dclskb=dclef(kb)*ctest
c gaz 042124 adding upwinded saturation derivative
         fid =0.5d0
         if((cnlf(kb)-cnli).gt.0.0d0) fid = 1.0d0
         if((cnlf(kb)-cnli).lt.0.0d0) fid = 0.0d0
         fid1=1.0-fid
         sat_avg = fid*s(kb)+fid1*s(i) 
         dsatavgpi = 0.0
         dsatavgpkb = 0.0
         dsatavgsi = fid1
         dsatavgskb = fid                                                           
