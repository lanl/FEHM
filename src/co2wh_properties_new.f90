subroutine co2wh_properties_new(iflg,iphase,var1,var2,var3,istate,var4,var5,var6)
!***********************************************************************
...
! gaz 083021 modified from air_properties new
  ...
  if (iflg .eq. 1) then
     ! determine co2 state
     call get_co2wh_state(ifail,temperature,pressure,istate)
     if(ifail.ne.0) go to 9890
  elseif(iflg.eq.2) then
     ! determine sat temperature given pressure
     call get_co2wh_sat_temperature(ifail,pressure,var4,var5(1))
  elseif(iflg.eq.3) then
     ! determine sat pressure given temprature
     call get_co2wh_sat_pressure(ifail,temperature,var4,var5(1))
  elseif(iflg.eq.4) then
     ! determine properties and derivatives for liquid,vapor,supercritical phases
     icode=1   ! an array
     if(iphase.eq.4) iphase=1
     call get_co2wh_properties(ifail,iphase,9,icode,temperature,pressure,var5)
  ...
  elseif(iflg.eq.5) then
     ...
     icode=1   ! an array
     call get_co2wh_sat_line_props_pressure(ifail,1,9,icode,pressure,var5)
     var5(4)=var5(4)*1.d-3
     var5(5)=var5(5)*1.d-3
     var5(6)=var5(6)*1.d-3
     var5(7)=var5(7)
  ...
  elseif(iflg.eq.6) then
     icode=1   ! an array
     call get_co2wh_sat_line_props_pressure(ifail,2,9,icode,pressure,var5)
     var5(4)=var5(4)*1.d-3
     var5(5)=var5(5)*1.d-3
     var5(6)=var5(6)*1.d-3
     var5(7)=var5(7)
...
end subroutine co2wh_properties_new
