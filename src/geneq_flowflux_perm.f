      subroutine geneq_flowflux_perm()
!***********************************************************************
...
! pressure from effective stress
      ...
      do i=1, neq
        ...
        call flow_flux_perm_residual(Ri, node_I, 0, 0.0d0, 0.0d0, 0.0d0
     &       ,0.0d0,0)
c s kelkar sep 5 2012
c        bp(i+nrhs(1)) = Ri(1)
c        bp(i+nrhs(2)) = Ri(2)
        ...
        do j=i_begin, i_end
          ...
          call flow_flux_perm_residual(Rjup, node_I, node_J, u_eps, 
     &         0.0d0, 0.0d0, 0.0d0, 1)
          call flow_flux_perm_residual(Rjvp, node_I, node_J, 0.0d0, 
     &         u_eps, 0.0d0, 0.0d0, 1)
          call flow_flux_perm_residual(Rjwp, node_I, node_J, 0.0d0, 
     &         0.0d0, u_eps, 0.0d0, 1)
          call flow_flux_perm_residual(Rjpp, node_I, node_J, 0.0d0, 
     &         0.0d0, 0.0d0, u_eps, 2)
...
c          goto 81811
          ...
          a(iau + nmat(5)) = a(iau + nmat(5)) + (Rjup(1) - Ri(1))/u_eps
          a(iau + nmat(6)) = a(iau + nmat(6)) + (Rjvp(1) - Ri(1))/u_eps
          a(iau + nmat(7)) = a(iau + nmat(7)) + (Rjwp(1) - Ri(1))/u_eps
          a(iau + nmat(3)) = a(iau + nmat(3)) + (Rjpp(2) - Ri(2))/u_eps
          a(iau + nmat(8)) = a(iau + nmat(8)) + (Rjup(2) - Ri(2))/u_eps
          a(iau + nmat(9)) = a(iau + nmat(9)) + (Rjvp(2) - Ri(2))/u_eps
          a(iau + nmat(10))= a(iau + nmat(10)) + (Rjwp(2) - Ri(2))/u_eps

81811     continue
          goto 8112

          a(iau + nmat(1)) = (Rjpp(1) - Ri(1))/u_eps
          a(iau + nmat(5)) = (Rjup(1) - Ri(1))/u_eps
          a(iau + nmat(6)) = (Rjvp(1) - Ri(1))/u_eps
          a(iau + nmat(7)) = (Rjwp(1) - Ri(1))/u_eps
          a(iau + nmat(3)) = (Rjpp(2) - Ri(2))/u_eps
 ...
 8112     continue
      ...
      end subroutine geneq_flowflux_perm
