      subroutine sub_gmres62(neq,a,b,r,na,nb,nrhs
     *     ,ncon,nop,north,sorthm,epn
     *     ,irb,iirb,npvt,rw,xx,xtemp,dum,piv
     *     ,h,c,s,g,y,iter,idof,iptty,icoupl,tollr,overf)
!***********************************************************************
...
CD1 acceleration (six degrees of freedom).
CD1
C**********************************************************************
CD2
...
CD2 6-24-94      B. Robinson    97      Converted 4 dof version to 6
CD2
CD2 $Log:   /pvcs.config/fehm90/src/sub_gmres62.f_a  $
!D2 
!D2    Rev 2.5   06 Jan 2004 10:44:10   pvcs
!D2 FEHM Version 2.21, STN 10086-2.21-00, Qualified October 2003
!D2 
!D2    Rev 2.4   29 Jan 2003 09:19:46   pvcs
!D2 FEHM Version 2.20, STN 10086-2.20-00
!D2 
!D2    Rev 2.3   14 Nov 2001 13:15:34   pvcs
!D2 FEHM Version 2.12, STN 10086-2.12-00
!D2 
!D2    Rev 2.2   06 Jun 2001 13:27:44   pvcs
!D2 FEHM Version 2.11, STN 10086-2.11-00
!D2 
!D2    Rev 2.1   30 Nov 2000 12:11:52   pvcs
!D2 FEHM Version 2.10, STN 10086-2.10-00
!D2 
!D2    Rev 2.0   Fri May 07 14:46:50 1999   pvcs
!D2 FEHM Version 2.0, SC-194 (Fortran 90)
CD2 
CD2    Rev 1.9   Thu Sep 19 12:22:04 1996   llt
CD2 added log history
CD2
CD2    Rev 1.8   09/17/96 09:33:16   robinson
CD2 Changed spelling of orthogalization
CD2
CD2    Rev 1.7   09/16/96 12:12:18   robinson
CD2 Oh, those nasty prologs
CD2
CD2    Rev 1.6   09/16/96 11:51:44   robinson
CD2 Prolog change
CD2
CD2    Rev 1.5   09/12/96 08:26:30   robinson
CD2 Prolog Changes
CD2
CD2    Rev 1.4   05/31/96 15:38:24   gaz
CD2 correction for more general dof reordering
CD2
CD2    Rev 1.3   05/14/96 14:32:52   hend
CD2 Updated output
CD2
CD2    Rev 1.2   02/02/96 12:39:32   hend
CD2 Updated Requirements Traceability
CD2
CD2    Rev 1.1   01/17/96 10:13:50   llt
CD2 changed type on max function for ibm
CD2
CD2    Rev 1.0   01/16/96 14:48:32   llt
...
CD4 sub_bksub62      N/A       Performs back-substitution, 6-2 dof
...
CPS sub_bksub6 - obtain new pre-conditioned solution
CPS 
...
CPS     ENDFOR
CPS     
CPS     sub_bksub6 - compute new pre-conditioned solution
CPS     
...
CPS     ENDFOR
CPS   
CPS     sub_bksb6 - obtain new pre-conditioned solution
CPS   
CPS   ENDIF
CPS 
...
c     note dimension of h is 1
      ...
      real*8 tols, rnorm
      real*8 vhatnm, tmp1, s1, s2, s3, s4, s5, s6, prdinr
      real*8 tmp2, sqroot, sum1, sum2, sum3, sum4, sum5, sum6, hh, summ
      integer neqm1, neqp1, maxit, inorth, kk, i
      integer nr1, nr2, nr3, nr5, nr6, lu3, k1, k2, jnorm1
      integer k3, nel, k5, k6
...
c     nrhs_dum(1)=0
c     nrhs_dum(2)=neq
c     nrhs_dum(3)=neq+neq
c     nrhs_dum(4)=neq+neq+neq
c     nrhs_dum(5)=neq+neq+neq+neq
c     nrhs_dum(6)=neq+neq+neq+neq+neq
      nrhs_dum(1)=nr1 
      nrhs_dum(2)=nr2 
      nrhs_dum(3)=nr3                   
      nrhs_dum(4)=nr4                   
...
c     
      call sub_bksub62(neq,a,b,r,na,nb,nrhs,ncon,nop
     *     ,npvt,piv,dum,icoupl,tollr,overf)
c     
c     
*------------------------------------------------------
*     gmres - six degrees of freedom.
*     
...
c     compute v1 in natural order
      do 1050 i=1,nel
         ...
         sorthm(i+jnel5)=dum(i+nrhs_dum(5))/rnorm
         sorthm(i+jnel6)=dum(i+nrhs_dum(6))/rnorm
 1050 continue
      jnorth=1
 1100 continue
...
c     compute av
 ...
 1140    continue
         ...
         dum(i+nrhs_dum(5))=sum5
         dum(i+nrhs_dum(6))=sum6
 1160 continue
*     -----------
*     reorder av
*     -----------
...
c     compute m(-1)*av
      call sub_bksub62(neq,a,b,rw,na,nb,nrhs,ncon,nop
     *     ,npvt,piv,dum,icoupl,tollr,overf)
c
*     ----------------------------------
...
c     
     ...
     &           +dum(i+nrhs_dum(5))*sorthm(k5+i)
     &           +dum(i+nrhs_dum(6))*sorthm(k6+i)
 1420    continue
...
c     
      do 1630 inor=1,jnorth
         ...
         do 1620 i=1,nel
            ...
            dum(i+nrhs_dum(5))=dum(i+nrhs_dum(5))-hh*sorthm(i+k5)
            dum(i+nrhs_dum(6))=dum(i+nrhs_dum(6))-hh*sorthm(i+k6)
 1620    continue
 1630 continue
c     compute ||v-hat||
...
c     compute next v
      ...
      do 1820 i=1,nel
         ...
         sorthm(k5+i)=dum(i+nrhs_dum(5))/vhatnm
         sorthm(k6+i)=dum(i+nrhs_dum(6))/vhatnm
 1820 continue
c     q-r factorization
 ...
 1910 continue
      ...
      if (abs(g(jnorth+1)).le.epn) then
         ...
         if(iter.gt.maxit) then
            if( iptty .ne. 0 ) then
               write(iptty,34)
 34            format(/,1x,'Warning issued by 6-2 degree of freedom '
     &              ,'solver:')
 ...
 2220 continue
      do 2350 i=1,nel
         ...
         do 2330 inor=1,inorth
            ...
            sum5=sum5+sorthm(i+k5)*y(inor)
            sum6=sum6+sorthm(i+k6)*y(inor)
 2330    continue
         xx(i,1)=xx(i,1)+sum1
         xx(i,2)=xx(i,2)+sum2
         xx(i,3)=xx(i,3)+sum3
...
c     form r = xtemp - axx (in reordered form)
     ...
     &              +a(ijaind+na15)*xx(ja,5)
     &              +a(ijaind+na16)*xx(ja,6)
               sum2=sum2
     &              +a(ijaind+na21)*xx(ja,1)
     &              +a(ijaind+na22)*xx(ja,2)
     &              +a(ijaind+na23)*xx(ja,3)
     &              +a(ijaind+na24)*xx(ja,4)
     &              +a(ijaind+na25)*xx(ja,5)
     &              +a(ijaind+na26)*xx(ja,6)
               sum3=sum3
     &              +a(ijaind+na31)*xx(ja,1)
     &              +a(ijaind+na32)*xx(ja,2)
     &              +a(ijaind+na33)*xx(ja,3)
     &              +a(ijaind+na34)*xx(ja,4)
     &              +a(ijaind+na35)*xx(ja,5)
     &              +a(ijaind+na36)*xx(ja,6)
               sum4=sum4
     &              +a(ijaind+na41)*xx(ja,1)
     &              +a(ijaind+na42)*xx(ja,2)
     &              +a(ijaind+na43)*xx(ja,3)
     &              +a(ijaind+na44)*xx(ja,4)
     &              +a(ijaind+na45)*xx(ja,5)
     &              +a(ijaind+na46)*xx(ja,6)
               sum5=sum5
     &              +a(ijaind+na51)*xx(ja,1)
     &              +a(ijaind+na52)*xx(ja,2)
     &              +a(ijaind+na53)*xx(ja,3)
     &              +a(ijaind+na54)*xx(ja,4)
     &              +a(ijaind+na55)*xx(ja,5)
     &              +a(ijaind+na56)*xx(ja,6)
               sum6=sum6
     &              +a(ijaind+na61)*xx(ja,1)
     &              +a(ijaind+na62)*xx(ja,2)
     &              +a(ijaind+na63)*xx(ja,3)
     &              +a(ijaind+na64)*xx(ja,4)
     &              +a(ijaind+na65)*xx(ja,5)
     &              +a(ijaind+na66)*xx(ja,6)
 2440       continue
*     -----------------------------------------
...
*     ------------------------------------------
            ...
            r(iirb(i)+nrhs(5))=xtemp(i+nrhs_dum(5))-sum5
            r(iirb(i)+nrhs(6))=xtemp(i+nrhs_dum(6))-sum6
 2460    continue
c     compute m(-1)*av
      call sub_bksub62(neq,a,b,r,na,nb,nrhs,ncon,nop
     *     ,npvt,piv,dum,icoupl,tollr,overf)
