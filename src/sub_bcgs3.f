      subroutine sub_bcgs3(neq,a,b,r,na,nb,nrhs
...
CD1 acceleration (three degrees of freedom).
CD1
C**********************************************************************
CD2
...
CD2                                     
CD2 $Log:   /pvcs.config/fehm90/src/sub_bcgs3.f_a  $
!D2 
!D2    Rev 2.5   06 Jan 2004 10:44:04   pvcs
!D2 FEHM Version 2.21, STN 10086-2.21-00, Qualified October 2003
!D2 
!D2    Rev 2.4   29 Jan 2003 09:18:26   pvcs
!D2 FEHM Version 2.20, STN 10086-2.20-00
!D2 
!D2    Rev 2.3   14 Nov 2001 13:15:02   pvcs
!D2 FEHM Version 2.12, STN 10086-2.12-00
!D2 
!D2    Rev 2.2   06 Jun 2001 13:27:16   pvcs
!D2 FEHM Version 2.11, STN 10086-2.11-00
!D2 
!D2    Rev 2.1   30 Nov 2000 12:10:58   pvcs
!D2 FEHM Version 2.10, STN 10086-2.10-00
!D2 
!D2    Rev 2.0   Fri May 07 14:46:18 1999   pvcs
!D2 FEHM Version 2.0, SC-194 (Fortran 90)
CD2 
CD2    Rev 1.5   Mon Sep 16 11:45:14 1996   robinson
CD2 Prolog change
CD2 
CD2    Rev 1.4   Thu Sep 12 08:25:56 1996   robinson
CD2 Prolog Changes
CD2 
CD2    Rev 1.3   Fri May 31 15:40:58 1996   gaz
CD2 correction for more general dof reordering
CD2 
CD2    Rev 1.2   Tue May 14 14:32:32 1996   hend
CD2 Updated output
CD2 
CD2    Rev 1.1   Fri Feb 02 12:19:34 1996   hend
CD2 Updated Requirements Traceability
CD2 
CD2    Rev 1.0   Tue Jan 09 14:14:48 1996   llt
...
c bcgs acceleration routine
      ...
      real*8 tols, sum1, sum2, sum3, beta_n          
      real*8 ad1,ad2,ad3,ad4,ad5,ad6,ad7,ad8,ad9
...
c
      call sub_bksub3(neq,b,r,nb,nrhs,nop
     *     ,npvt,piv)
...
*     ----------------------------------
     ...
     2                   + ad3*sorthm(lu2+neq2+kb)
             sum2 = sum2 + ad4*sorthm(lu2+kb) + ad5*sorthm(lu2+neq+kb)
     2                   + ad6*sorthm(lu2+neq2+kb)
             sum3 = sum3 + ad7*sorthm(lu2+kb) + ad8*sorthm(lu2+neq+kb)
     2                   + ad9*sorthm(lu2+neq2+kb)
            enddo
            xtemp(i) = sum1
            xtemp(i+neq) = sum2
            xtemp(i+neq2) = sum3
...
*     ----------------------------------
     ...
     2                   + ad3*xtemp(neq2+kb)
             sum2 = sum2 + ad4*xtemp(kb) + ad5*xtemp(neq+kb)
     2                   + ad6*xtemp(neq2+kb)
             sum3 = sum3 + ad7*xtemp(kb) + ad8*xtemp(neq+kb)
     2                   + ad9*xtemp(neq2+kb)
            enddo
            sorthm(i) = sum1
            sorthm(i+neq) = sum2
...
c
      ...
      if (iter .gt. maxit) then
         if(iptty .ne. 0 ) then
            write(iptty,34)
 34         format(/,1x,'Warning issued by 3 degree of freedom '
     &           ,'solver:')
