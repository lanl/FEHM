      subroutine geneqc_w_nondarcy(i)
!***********************************************************************
...
c gaz 131025
c gaz 131025 moving  variables to com_darcy_AWH
      real*8  acxy
      real*8  aexy, alxi, alxkb, alyi, alykb, alzi, alzkb
      real*8  avxi, avyi, avzi, axi, axkb, axy, axyd
      real*8  ayi, aykb, azi, azkb
c      real*8  cnli, cnlkb, cnvi, cnvkb
c      real*8  dclei, dclci, dclckb, dclekb, dcli, dclkb
c      real*8  dcvei, dcvci, dcvckb, dcvekb
      real*8  ddva_barci, ddva_barckb, ddva_barei, ddva_barekb 
      real*8  ddva_barpi, ddva_barpkb
c      real*8  delci, delckb 
      real*8  delx2, dely2, delz2
      real*8  devmci, devmei, devmpi
      real*8  dis2,dis_tol,sx_min 
c      real*8  dilci, dilckb
c      real*8  dilei, dilekb, dili, dilkb, dilpi, dilpkb
c 103125 (removed a line)
      ...
      real*8  grav_air, heatc 
c 103125
      real*8  envmi, envmkb
c gaz 300925 these now passed therough   com_nondarcy_AWH
c     real*8  pxy, pxyi, sx4d, fid, fid1 
      real*8  phii, phikb, pvii, pvikb, pvxy, pxyh
      real*8  psat, qhz, radi, radkb, skz
      real*8  swi, sx1d, sx2c, sx2t, sx3c, sx3t, sx4h, sxzt
      real*8  thxi, thxkb, thyi, thykb, thzi, thzkb, ti
c  103125    real*8  vcxy, vcxyf, vexy, vexyf, vxy, vxyd, vxyf, water_pressure
      real*8  vcxy,  vexy, vxyd, vxy,  water_pressure
...
c gaz 300925
     ...
     *        *(cord(kz,igrav)-cord(iz,igrav))
         t8(neighc)=axyd
c gaz debug 081225 
c            write(*,*)  i,kb,axyd,axyd_nd,phi(1),phi(2),s(i)
c            pause   'before nd_flow_AWH_liquid(1'
c  calculate grav derivatives for axyd (not yet ND deriv)
         call nd_flow_AWH_liquid(1,jm,neighc,iz,i,kz,kb,axyd,axyd_nd,
     &                           fid)
c gaz debug 081225 
c            write(*,*)  i,kb,axyd,axyd_nd,phi(1),phi(2),s(i)
c            pause   'after nd_flow_AWH_liquid(1'
c
c 60   continue combined 60,61 do loops
c
c major changes
c
            fid=0.5
            ...
            fid1 = 1.d0-fid
            t9(neighc)=fid
c calculate some fluid props for ND terms
            call nd_props(0,icd,1,iq,i,kb,jm,0.5d0)
c calculate ND flow rates
            call nd_flow_AWH_liquid(2,jm,neighc,iz,i,kz,kb,axyd,axyd_nd,
     &                           fid)
c gaz debug 081225 
c            write(*,*)  i,kb,axyd,axyd_nd,phi(1),phi(2),s(i)
c            write(*,*) 'isl', isl
c            pause  'nd_flow_AWH_liquid(2'
            t8_nd(neighc)=axyd_nd
            fid=.5d0
            if(axyd_nd.lt.0.0) fid=dnwgt
            if(axyd_nd.gt.0.0) fid=upwgt
            t9_nd(neighc)=fid
            call nd_flow_AWH_liquid(3,jm,neighc,iz,i,kz,kb,axyd,axy_nd,
     &                           fid)
 60   continue
c
c form equations
...
c gaz 201025
           call nd_flow_AWH_liquid(4,jm,neighc,iz,i,kz,kb,axyd,axy_nd,
     &                           fid)
            axy_nd=axyd_nd*axyf
            ...
            aexy = aexy_nd
            acxy = acxy_nd
            a_axy(iau+nmatavw)=axyd_nd*(fid*dilkb+fid1*dili)
            a_axy(ial+nmatavw)=-a_axy(iau+nmatavw)
            dlapi  = dlapi_nd 	
	  	   dlapkb = dlapkb_nd	
	  	   dlaei  = dlaei_nd    
            ...
            dlcekb = dlcekb_nd   
	  	   ...
	  	   dlcckb = dlcckb_nd
c gaz debug 081225 
c            write(*,*)  iz,i,kz,kb,dlapi_nd,dleei_nd,dlcei_nd 
c            pause  'after derivatives'            
...
c initialize grav_wgt
     ...
     &        *(cord(kz,igrav)-cord(iz,igrav))
         t8(neighc)=vxyd
c  calculate grav derivatives for axyd (not yet ND deriv)
         call nd_flow_AWH_vapor(1,jm,neighc,iz,i,kz,kb,vxyd,vxyd_nd,fid)         
c 63   continue combined loops
c
c major changes
c
            fid=0.5
...
c calculate some fluid props for ND terms
            call nd_props(0,icd,1,iq,i,kb,jm,0.5d0)
c calculate ND flow rates
            call nd_flow_AWH_vapor(2,jm,neighc,iz,i,kz,kb,vxyd,vxyd_nd,
     &                            fid)
            ...
            t9_nd(neighc)=fid
            call nd_flow_AWH_vapor(3,jm,neighc,iz,i,kz,kb,vxyd,vxy_nd,
     &                            fid)
c            call nd_flow_AWH_v(4,jm,neighc,iz,i,kz,kb,vxyd,vxy_nd,fid)
 63   continue
...
c gaz 311025
           call nd_flow_AWH_vapor(4,jm,neighc,iz,i,kz,kb,vxyd,vxy_nd,
     &                            fid)
            vxy_nd=vxyd_nd*vxyf
            ...
            vexy = vexy_nd
            vcxy = vcxy_nd
            a_vxy(iau+nmatavw)=vxyd_nd*(fid*divkb+fid1*divi)
            a_vxy(ial+nmatavw)=-a_vxy(iau+nmatavw)
            dvapi  = dvapi_nd 	
	  	   dvapkb = dvapkb_nd	
	  	   dvaei  = dvaei_nd    
...
4            bp(iz+nrhs(1))=bp(iz+nrhs(1))+vxy
...
c gaz 311025 vxyd is a dummy variable for determing upwind
                     fid=.5
