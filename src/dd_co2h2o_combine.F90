#ifdef DDFLAG
#ifdef DDFLAG
	subroutine dd_co2h2o_combine(iflg,idofm)
!***********************************************************************
...
C***********************************************************************
      ...
      PetscInt iflg, idofm, nmatd, index(6)
      PetscInt i, icoupl_iter, id, idl 
...
c     RJP 12/23/04 change below later
            do i = dd_neq_start, dd_neq_end
               i1 = nelm(i)+1
               i2 = nelm(i+1)
               do j = i1, i2
                  if (ices(i).eq.2) then
                     a(j+nmat(2)-(dd_neq+1)) = a(j+nmat(4)-(dd_neq+1))
                     a(j+nmat(6)-(dd_neq+1)) = a(j+nmat(8)-(dd_neq+1))
                     a(j+nmat(10)-(dd_neq+1)) = a(j+nmat(12)-(dd_neq+1))
                  endif
               enddo
            enddo
...
c     RJP 12/23/04 change below later
            do i = 1, dd_neq
               ...
               do j = i1, i2
                  if (ices(i).ne.2) then
                     a(j+nmat(3)-(dd_neq+1)) = a(j+nmat(4)-(dd_neq+1))
                     a(j+nmat(4)-(dd_neq+1)) = a(j+nmat(5)-(dd_neq+1))
                  else
                     a(j+nmat(2)-(dd_neq+1)) = a(j+nmat(3)-(dd_neq+1))
                     a(j+nmat(3)-(dd_neq+1)) = a(j+nmat(4)-(dd_neq+1))
                     a(j+nmat(4)-(dd_neq+1)) = a(j+nmat(6)-(dd_neq+1))
                  endif
               enddo
            enddo
