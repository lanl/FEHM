      subroutine sub_gmres2(neq,a,b,r,na,nb,nrhs,ncon,nop
     *     ,north,sorthm,epn
...
CD2                                     
CD2 $Log:   /pvcs.config/fehm90/src/sub_gmres2.f_a  $
!D2 
!D2    Rev 2.5   06 Jan 2004 10:44:08   pvcs
!D2 FEHM Version 2.21, STN 10086-2.21-00, Qualified October 2003
!D2 
!D2    Rev 2.4   29 Jan 2003 09:19:36   pvcs
!D2 FEHM Version 2.20, STN 10086-2.20-00
!D2 
!D2    Rev 2.3   14 Nov 2001 13:15:22   pvcs
!D2 FEHM Version 2.12, STN 10086-2.12-00
!D2 
!D2    Rev 2.2   06 Jun 2001 13:27:32   pvcs
!D2 FEHM Version 2.11, STN 10086-2.11-00
!D2 
!D2    Rev 2.1   30 Nov 2000 12:11:36   pvcs
!D2 FEHM Version 2.10, STN 10086-2.10-00
!D2 
!D2    Rev 2.0   Fri May 07 14:46:40 1999   pvcs
!D2 FEHM Version 2.0, SC-194 (Fortran 90)
CD2 
CD2    Rev 1.8   Tue Sep 17 09:32:54 1996   robinson
CD2 Changed spelling of orthogalization
CD2 
CD2    Rev 1.7   Mon Sep 16 12:11:32 1996   robinson
CD2 Oh, those nasty prologs
CD2 
CD2    Rev 1.6   Thu Sep 12 08:26:20 1996   robinson
CD2 Prolog Changes
CD2 
CD2    Rev 1.5   Fri May 31 15:36:18 1996   gaz
CD2 correction for more general dof reordering
CD2 
CD2    Rev 1.4   Tue May 14 14:32:38 1996   hend
CD2 Updated output
CD2 
CD2    Rev 1.3   Fri Feb 02 12:28:12 1996   hend
CD2 Updated Requirements Traceability
CD2 
CD2    Rev 1.2   05/11/94 16:19:02   llt
CD2 bug fixes - gaz
CD2 
CD2    Rev 1.1   03/18/94 16:05:34   gaz
CD2 Added solve_new and cleaned up memory management.
CD2 
CD2    Rev 1.0   03/02/94 08:46:48   pvcs
...
CD4 sub_bksub2      N/A       Performs back-substitution, 2 dof
...
CPS sub_bksub2 - obtain new pre-conditioned solution
CPS 
...
CPS     
CPS     renumber_array - renumber solution array
CPS     sub_bksub2 - compute new pre-conditioned solution
CPS     renumber_array - renumber solution array
CPS     
...
CPS   ENDFOR
CPS   
CPS   sub_bksb2 - obtain new pre-conditioned solution
CPS 
CPS ENDLOOP
CPS 
CPS FOR each equation
...
c     acceleration routine
      ...
      integer neq, maxor
      real*8 xx(neq,2),a(*),b(*),r(*),sorthm(*),dum(*),rw(*)
      integer ncon(*),nop(*)
      real*8 xtemp(*),piv(neq,4)
      integer irb(*),iirb(*),npvt(*), na(*), nb(*)
      real*8 h(maxor,*),c(*),s(*),g(*),y(*), epn
      integer nrhs(*), north, iter, idof, iptty
      real*8 tols, rnorm, sum, ad1, ad2, ad3, ad4, vhatnm, tmp1
      real*8 tmp2, sqroot, sum1, sum2, dum1, dum2
...
c     
     ...
     *     ,npvt,piv)         
c     
c     arrange solution in natural order
c     
*------------------------------------------------------
*     gmres - two degrees of freedom.
*     
...
*     -----------
         do 2100 i = 1,neq
            ...
            do 2000 jj = j1,j2
               ...
               ad3 = a(jj - neqp1 + na(3))
               ad4 = a(jj - neqp1 + na(4))
               sum1 = sum1 + ad1*sorthm(ke0+kb) + ad2*sorthm(ke1+kb)
               sum2 = sum2 + ad3*sorthm(ke0+kb) + ad4*sorthm(ke1+kb)
 2000       continue
...
c     back substitution
c     
         call sub_bksub2(neq,b,rw,nb,nrhs_dum,nop
     *        ,npvt,piv)         
c     
...
*     -----------------
         ...
         h(j+1,j) = vhatnm
c     compare norm to machine tolerance
...
*     ---------------------------------------
      ...
      do 4200 i = 1,neq
         ...
         do 4100 jj = j1,j2
            ...
            ad3 = a(jj - neqp1 + na(3))
            ad4 = a(jj - neqp1 + na(4))
            sum1 = sum1 + ad1*xx(kb,1) + ad2*xx(kb,2)
            sum2 = sum2 + ad3*xx(kb,1) + ad4*xx(kb,2)
 4100    continue
         dum(i+nrhs_dum(1)) = sum1
         dum(i+nrhs_dum(2)) = sum2
 4200 continue
