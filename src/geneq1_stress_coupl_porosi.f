      subroutine  geneq1_stress_coupl_porosi  ( i )
!***********************************************************************
...
C***********************************************************************
      ...
      use comsi, only:dvol_strainu, dvol_strainv, dvol_strainw
     &         ,density, internal_energy, ihms, pore_factor 
      implicit none
...
c
      real*8 fi_ukb, fi_vkb, fi_wkb, efi_ukb,efi_vkb,efi_wkb
      real*8 fkb_ui, fkb_vi, fkb_wi, efkb_ui, efkb_vi, efkb_wi
      real*8 fi_ui, fi_vi, fi_wi, efi_ui, efi_vi, efi_wi 
      real*8 fkb_ukb, fkb_vkb, fkb_wkb, efkb_ukb, efkb_vkb, efkb_wkb 
      real*8 factor, efactor, factorkb, efactorkb
...
c
           if(allocated(permfactor).and.ihms.eq.17) then
...
c!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
c derivatives of accumulations terms wrt displacement that result
c from the pore volume change

      factor = density(i)*pore_factor
      efactor= internal_energy(i)*pore_factor
      fi_ui=factor*dvol_strainu(jmia)
      fi_vi=factor*dvol_strainv(jmia)
      fi_wi=factor*dvol_strainw(jmia)
      efi_ui=efactor*dvol_strainu(jmia)
      efi_vi=efactor*dvol_strainv(jmia)
      efi_wi=efactor*dvol_strainw(jmia)
c derivative of mass equation wrt displacement node itself
      a(jmia+nmat(5))=a(jmia+nmat(5))+fi_ui
      a(jmia+nmat(6))=a(jmia+nmat(6))+fi_vi
      a(jmia+nmat(7))=a(jmia+nmat(7))+fi_wi
c derivative of energy equation wrt displacement node itself
      a(jmia+nmat(8))= a(jmia+nmat(8)) +efi_ui
      a(jmia+nmat(9))= a(jmia+nmat(9)) +efi_vi
      a(jmia+nmat(10))=a(jmia+nmat(10))+efi_wi
      ...
      do 71 jm=1,iq
         ...
         ial=it12(jm)
         jml=nelmdg(kz)-neqp1
         factorkb = density(kb)*pore_factor
         efactorkb= internal_energy(kb)*pore_factor
c 
         fi_ukb=factor*dvol_strainu(iau)
         fi_vkb=factor*dvol_strainv(iau)
         fi_wkb=factor*dvol_strainw(iau)

         efi_ukb=efactor*dvol_strainu(iau)
         efi_vkb=efactor*dvol_strainv(iau)
         efi_wkb=efactor*dvol_strainw(iau)

         fkb_ui=factor*dvol_strainu(ial)
         fkb_vi=factor*dvol_strainv(ial)
         fkb_wi=factor*dvol_strainw(ial)
c         fkb_ukb=factor*dvol_strainu(jml)
c         fkb_vkb=factor*dvol_strainv(jml)
c         fkb_wkb=factor*dvol_strainw(jml)

         efkb_ui=efactor*dvol_strainu(ial)
         efkb_vi=efactor*dvol_strainv(ial)
         efkb_wi=efactor*dvol_strainw(ial)
c         efkb_ukb=efactor*dvol_strainu(jml)
c         efkb_vkb=efactor*dvol_strainv(jml)
c         efkb_wkb=efactor*dvol_strainw(jml)
c mass equation accumulation term
         a(iau+nmat(5))=a(iau+nmat(5))+fi_ukb
         a(iau+nmat(6))=a(iau+nmat(6))+fi_vkb
         a(iau+nmat(7))=a(iau+nmat(7))+fi_wkb

         a(ial+nmat(5))=a(ial+nmat(5))+fkb_ui
         a(ial+nmat(6))=a(ial+nmat(6))+fkb_vi
         a(ial+nmat(7))=a(ial+nmat(7))+fkb_wi

c         a(jml+nmat(5))=a(jml+nmat(5))-fkb_ukb
c         a(jml+nmat(6))=a(jml+nmat(6))-fkb_vkb
c         a(jml+nmat(7))=a(jml+nmat(7))-fkb_wkb
c energy equation accumulation term     
         a(iau+nmat(8))= a(iau+nmat(8)) +efi_ukb
         a(iau+nmat(9))= a(iau+nmat(9)) +efi_vkb
         a(iau+nmat(10))=a(iau+nmat(10))+efi_wkb

         a(ial+nmat(8))= a(ial+nmat(8)) +efkb_ui
         a(ial+nmat(9))= a(ial+nmat(9)) +efkb_vi
         a(ial+nmat(10))=a(ial+nmat(10))+efkb_wi

c         a(jml+nmat(8))= a(jml+nmat(8)) -efkb_ukb
c         a(jml+nmat(9))= a(jml+nmat(9)) -efkb_vkb
c         a(jml+nmat(10))=a(jml+nmat(10))-efkb_wkb
