...
c loop to find other primary node connections (with gdpm nodes)   
	      kb = it10(jj)
c 	 areat = t1(jj)/vfrac  this is formulation if   
            areat = t1(jj)/vfrac 
            cord1j = cord(kb,1)
	      cord2j = cord(kb,2)
	      cord3j = 0.0
...
c adjust connectivity and area coefficients           
           do jj = minkb,maxkb
            if(idum(jj).ne.0) then
             ...
             nneigh(ii-neq_primary,nconi(ii-neq_primary)) = jj
             sx_gdkm(ii-neq_primary,nconi(ii-neq_primary)) = sxdum(jj)
             idum(jj) = 0
             sxdum(jj) = 0.0
            endif
           enddo
...
c correct size for nelm is nsize + ic
      nsize = nelm(neq_primary+1)
      i2 = nsize+ic
      allocate(nelm_temp(nsize))
      nelm_temp(1:nsize) = nelm(1:nsize) 
      deallocate (nelm)
      allocate (nelm(i2)) 
      nelm(1:nsize) = nelm_temp(1:nsize) 
...
c first calculate the added size (icw) 
c adjust for symmetry and no diag
      icw = ic - (neq-neq_primary) + 1
      if(isoy.eq.1) then
       allocate(sx_dum(nr+icw,1)) 
       do j = 1, nr
        sx_dum(j,1) = sx(j,1)
       enddo 
      else
       allocate(sx_dum(nr+icw,3))
       do j = 1, nr
        sx_dum(j,1) = sx(j,1)
        sx_dum(j,2) = sx(j,2)
        sx_dum(j,3) = sx(j,3) 
...
c  fill in symmetric coefficient matrix
      allocate(istrw_gdkm(nsize-neqp1+icw))
       istrw_gdkm(1:nsize-neqp1) = istrw(1:nsize-neqp1)
       deallocate(istrw)
       ic = nr
c upper diagonal only 
      sxdum = 0.0d0
      do i = neq_primary+1, neq  
       i1 = nelmdg(i)+1
       i2 = nelm(i+1)
       ...
       enddo
      enddo   
       allocate(istrw(nsize-neqp1+icw))
       istrw = istrw_gdkm
       deallocate(istrw_gdkm)
c 
      if(isoy.eq.1) then
       allocate(sx(nr+icw,1)) 
       do j = 1, ic
        sx(j,1) = sx_dum(j,1)
       enddo 
      else
       allocate(sx(nr+icw,3))
       do j = 1, ic
        sx(j,1) = sx_dum(j,1) 
        sx(j,2) = sx_dum(j,2)
        sx(j,3) = sx_dum(j,3)
