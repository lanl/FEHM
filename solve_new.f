      subroutine solve_new(neq,a,b,r,na,nb,nrhs,ncon,nop,inorth,epn
...
CD1 linear, algebraic equations.
CD1
C**********************************************************************
CD2
...
CD2                                     
CD2 $Log:   /pvcs.config/fehm90/src/solve_new.f_a  $
!D2 
...
!D2 
!D2    Rev 2.1   30 Nov 2000 12:10:20   pvcs
...
C**********************************************************************
      ...
      integer na(*),nb(*),nrhs(*)
      integer inorth, north, iter, iback, iptty
      integer isolve
      integer ibcgs
      character*4 accm
...
c
      ...
      elseif (accm.eq.'gmre') then
       ibcgs = 0
      elseif (accm.eq.'    ') then
        write(iptty,*) 'ERROR in Solve : accm not defined, stopping' 
        stop
      endif
      if( idof .gt. 6 ) then
         isolve = 1
...
c
      if(anorm.le.epn) then
         ...
         if (iptty .ne. 0) then
            write(iptty,*) '  '
            write(iptty,*) 
     &           'one iteration done because solution is converged'
         end if
c        epn=anorm*0.1d00    
         epn = 0.0d00
         return
